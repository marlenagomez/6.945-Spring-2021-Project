...
;disp procedure from:  http://courses.csail.mit.edu/6.034s/04/scheme.html
...
(define (holes let-vars let-body)
  (if (> (length let-vars) 0)
      (let ((set-diff (set-difference (cadar let-vars) (caddr let-body))))
	(if (= (length set-diff) 1)
	    (let ((name (generate-uninterned-symbol "expr-"))
		  (set-sim (set-similarity (cadar let-vars) (caddr let-body))))
	      (disp "algebraic expressions: " (cadar let-vars) " and " (caddr let-body)
		    " \nhave one parameter different, so can be combined into\n")
	      (disp "(define (" (symbol->string name) " v) " set-sim ")\n"))))))
...
(define (set-similarity s1 s2)
  (cond ((null? s1)
	 '())
	((member (car s1) s2)
	 (cons (car s1) (set-similarity (cdr s1) s2)))

	(else
	 (cons 'v (set-similarity (cdr s1) s2)))))
...
(pp (gjs/cselim '(+ (* x 3) (* y 3) (* x 3))))
#|
Output:
(* x 3)
algebraic expressions: (* x 3) and (* y 3)
have one parameter different, so can be combined into:
(define (expr-51 v) (* v 3))
"Common subexpression(s) found! add-to-library?"
|#

(pp (gjs/cselim '(- (- x y z) (- x y w) (- x y z))))
#|
Output:
(- x y z)
algebraic expressions: (- x y z) and (- x y w)
have one parameter different, so can be combined into:
(define (expr-47 v) (- x y v))
"Common subexpression(s) found! add-to-library?"
|#
